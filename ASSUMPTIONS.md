...
- SSH automation accounts execute the host CLI from `/usr/local/bin/moltbox` with `/etc/moltbox/config.yaml`, while the operator-facing install remains `~/.local/bin/moltbox`.
