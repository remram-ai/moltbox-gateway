...
- the host-side directories also were not usable git checkouts for a normal `pull exact revision` flow
...
### 3. `--session-id` isolation is not trustworthy
...
### 4. No safe official session reset surface
...
### 5. Tool-call observability in normal CLI output is weak
...
### 6. Runtime verify was baseline-stale before this change
...
### 7. ZFS snapshot-first recovery is not fully reachable from the AI test operator identity
...
### 8. Default `thinking` is not yet a managed runtime knob
...
2. Add an official “clear runtime sessions” operator surface for `test`.
3. Expose tool-call traces more clearly in the standard `agent --json` operator path.
4. Treat returned session identity as a bug until it matches requested `--session-id` isolation.
5. Decide whether default `thinking off` should become a managed baseline knob or remain a per-run override.
