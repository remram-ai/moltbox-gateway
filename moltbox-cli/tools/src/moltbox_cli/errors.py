...
@dataclass
class MoltboxCliError(Exception):
    error_type: str
    error_message: str
...
class ConfigError(MoltboxCliError):
...
class ValidationError(MoltboxCliError):
    def __init__(self, error_message: str, recovery_message: str, **details: Any) -> None:
        super().__init__(
...
class TargetNotFoundError(MoltboxCliError):
    def __init__(self, target: str) -> None:
        super().__init__(
            error_type="target_not_found",
            error_message=f"target '{target}' not registered",
            recovery_message="run `moltbox tools inspect` to see available targets",
