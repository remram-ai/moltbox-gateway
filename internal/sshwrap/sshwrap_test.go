...
func TestResolveAutomationPreservesQuotedArgs(t *testing.T) {
	...
	args, deny, err := Resolve(ModeAutomation, `moltbox test openclaw agent --message "Say hello in one sentence." --json`)
	if err != nil {
...
func TestResolveRejectsShellOperators(t *testing.T) {
	...
	_, _, err := Resolve(ModeAutomation, `moltbox test openclaw health --json; whoami`)
	if err == nil {
