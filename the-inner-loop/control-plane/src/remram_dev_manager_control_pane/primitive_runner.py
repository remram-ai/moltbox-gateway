...
PRIMITIVES: dict[str, PrimitiveDefinition] = {
    "render_assets": PrimitiveDefinition(
        ...
        "deploy/render-assets.py",
        False,
        ("target", "profile", "config_path", "state_root", "runtime_artifacts_root", "internal_host", "internal_port"),
    ),
    "inspect_target": PrimitiveDefinition(
        "inspect_target",
        "shared",
        "Inspect the current container state for a deploy-managed target.",
        "shared/inspect-target.py",
        False,
    ...
    "restore_target_snapshot": PrimitiveDefinition(
        ...
        "snapshot/restore-target-snapshot.py",
        True,
        ("target", "snapshot_id", "snapshot_dir", "compose_project", "container_names"),
    ),
    "validate_target": PrimitiveDefinition(
        "validate_target",
        "validate",
        "Run baseline container validation for a deploy-managed target.",
        "validate/validate-target.py",
        False,
...
def run_primitive(config: AppConfig, name: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if invalid_keys:
        raise RemramError(
            ...
            recovery_message="fix the control-plane primitive payload and rerun the command",
