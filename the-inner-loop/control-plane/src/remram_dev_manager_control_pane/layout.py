...
class RepoLayout:
    ...
    def as_dict(self) -> dict[str, str]:
        return {
            ...
            "inner_loop_dir": str(self.inner_loop_dir),
...
def build_repo_layout(repo_root: Path | None = None) -> RepoLayout:
    ...
    inner_loop_dir = root / "the-inner-loop"
    return RepoLayout(
        repo_root=root,
        inner_loop_dir=inner_loop_dir,
        control_pane_dir=inner_loop_dir / "control-plane",
        host_tools_source_dir=inner_loop_dir / "host-tools",
        docs_dir=inner_loop_dir / "docs",
    )
